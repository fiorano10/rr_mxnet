...
class RosMxNetSSD:
    ...
    def report_overlaps(self):
        ...
        rospy.logwarn("For Input Image Shape=%d,%d,%d, overlap first-level: %d%%, second-level zoom overlap: %d%% x %d%%", self.data_shape[0],self.data_shape[1],self.data_shape[2], int(100*level0_overlap), int(100*level1_xoverlap), int(100*level1_yoverlap))
            ...
            if self.last_detection_time+self.timer <= current_time:
                ...
                try:
                    ...
                    if (self.image_counter % 11) == 10:
                        rospy.loginfo("Images detected per second=%.2f", float(self.image_counter)/(time.time() - self.start_time))
